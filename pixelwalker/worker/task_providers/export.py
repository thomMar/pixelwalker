...
class ExportProvider(TaskProvider):
    ...
    def execute(self):
        ...
        if self.split_required == '1':
            self.output_file_path = os.path.join(output_directory, str('SPLIT_'+os.path.basename(self.input_file_path)))
            command = ['ffmpeg',
                    '-i', self.reference_file_path,
                    '-i', self.input_file_path,
                    '-an',
                    '-filter_complex', '[0]crop=iw/2:ih:0:0,pad=iw*2:ih[left];[1]scale='+self.reference_width+':'+self.reference_height+'[scaled];[scaled]crop=iw/2:ih:iw/2:0[right];[left][right]overlay=w[main],[main]select=between(n\,'+self.input_frame_in+'\,'+self.input_frame_out+'),setpts=PTS-STARTPTS,drawtext=text=''source'': fontcolor=white: fontsize=24 :box=1: boxcolor=black@0.5 :boxborderw=5:x=(w-text_w)/4.5:y=(h-text_h)/1.1,drawtext=text=''encoded'': fontcolor=white: fontsize=24 :box=1: boxcolor=black@0.5:boxborderw=5:x=(w-text_w)/1.4:y=(h-text_h)/1.1',
